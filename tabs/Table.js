...
function TableRow({ index }) {
    return (<View className="flex-row flex-1 py-2 border-t border-gray-700 -mx-4 ">
        <View className="flex-row px-4 ">
            <Text className="w-1/12  ">{index}</Text>
            <View className="w-8/12 flex-row  ">
                <Image className="w-6  h-6  object-contain" source={require('./../public/images/clubicon/chelsea.png')} />
                <Text className="ml-2">Inter</Text>

            </View>

            <Text className="w-1/12  text-center">15</Text>
            <Text className="w-1/12  text-center">30</Text>
            <Text className="w-1/12  text-center">38</Text>
        </View>
    </View>);
}
...
const Table = () => {
    ...
    return (
        <>
            <View className="border-[1px] border-gray-700 rounded-md w-full  my-3 flex flex-col   p-4">
